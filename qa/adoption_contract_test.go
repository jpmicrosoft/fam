...
func TestPromptWorkflowTemplateContract(t *testing.T) {
	...
	requireText(t, template,
		"id-token: write",
		"azure/login@v2",
		"foundry-agent-manager prompt validate",
		"foundry-agent-manager prompt preflight",
		"foundry-agent-manager prompt deploy",
		"--if-changed",
		"--output json",
		"--receipt",
...
func TestHostedWorkflowTemplateContract(t *testing.T) {
	...
	requireText(t, template,
		...
		"foundry-agent-manager hosted validate",
		"foundry-agent-manager hosted plan",
		"foundry-agent-manager hosted preflight",
		"args=(",
		"\n            hosted",
		"\n            deploy",
		`foundry-agent-manager "${args[@]}"`,
		"--accept-preview",
		"--if-changed",
	...
	for _, forbidden := range []string{
		"foundry-agent-manager prompt validate \\",
		"foundry-agent-manager prompt plan \\",
		"foundry-agent-manager prompt preflight \\",
		"foundry-agent-manager prompt deploy \\",
	} {
