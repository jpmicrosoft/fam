...
| **Agent ID blueprint** | A specialized Microsoft Entra application that acts as an identity and permissions template for agent identity instances. It is not agent code, instructions, a Prompt manifest, or a Hosted workspace. FAM cannot create a Foundry agent from it or attach it to an agent. |
...
| **Identity comparison (blueprint correlation)** | A read-only comparison between an existing Agent ID blueprint's IDs and the blueprint fields returned by Foundry. A match is correlation evidence only, not a created binding. The `binding status` and `binding plan` command names are retained for compatibility; no binding or apply operation is available. |
...
| **Plan** | Show proposed actions or analysis without intentionally changing resources. Prompt/Hosted deployment plans are offline. Not every plan has an executable follow-up: `agent365 binding plan` is read-only online identity comparison only, with no binding or apply operation. |
