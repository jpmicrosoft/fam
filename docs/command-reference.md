...
the current source tree (`0.16.1` is the prepared release version).
...
```powershell
fam version
# fam 0.16.1 commit=<commit> built=<timestamp>
```
...
An unstamped `go build` prints `fam 0.16.1` and `{"version": "0.16.1"}`.
